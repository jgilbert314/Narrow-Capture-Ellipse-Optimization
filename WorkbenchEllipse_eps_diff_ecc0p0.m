...
% User Input
% Domain setup
ecc = 0.0;                  % Eccentricity of ellipse
epsilon = 1/20;             % Trap size
epsilon_0 = epsilon;
N_p = 1;                    % Number of traps differing in size
...
% File name
log_dir = ['Ellipse\LogFiles\Log_eps-diff_ecc0p0', num2str(1000*ecc, 3)];
ref_logname = 'GoodLog20.csv';

flag_init = 1; % Generate first set of optimums, rather than read from file
if (flag_init)
    compData = {[NaN, NaN]}; % Dummy data
else
    compData = readLogFile( fullfile(log_dir, ref_logname) ); % Current best data
end
compN = getN(compData);
...
while ( ~isempty(N_list) && (optCount < optLimit) )
    ...
    try
        for itr = 1:nN
            ...
            InputStruct.N = N;
            InputStruct.epsilon = calcTwoTrapSizes(epsilon_0, N, N_p, k);
