...
InputStruct.init_search = 'gradient'; % Search algorithm to run on first pass ('global' or 'local')
InputStruct.flag_init = 0;       % Generate first set of optimums, rather than read from file
% File name
InputStruct.log_dir = ['Ellipse\Test\Log_eps0p', num2str(1000*epsilon, 3), '0-ecc0p', num2str(1000*ecc, 3)];
InputStruct.archive_base = 'GoodLog';
InputStruct.ref_filename = [InputStruct.log_dir, '\InitData.csv'];
% Log file init
...
InputStruct.dx = 1e-6;
GradStruct.thresh = InputStruct.diff_thresh;
GradStruct.iterLim = 1e4;
InputStruct.GradStruct = GradStruct;
